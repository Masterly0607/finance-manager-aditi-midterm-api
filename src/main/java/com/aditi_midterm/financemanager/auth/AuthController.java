...
public class AuthController {
  ...
  /**
   * Login: - returns access token in body (AuthResponse) - sets refresh token as HttpOnly cookie in
   * response
   */
  @PostMapping("/login")
  ...
  /**
   * Refresh: - browser sends refresh token cookie automatically - backend verifies it and returns a
   * NEW access token - optional: rotate refresh token and set a new cookie Note: Send refresh when
   * access token is expired or when we refresh page or close tab(these will delete access from
   * memory)
   */
  @PostMapping("/refresh")
  ...
  /**
   * Logout: - clears refresh token cookie - backend should revoke refresh token server-side
   * (db/redis) if you store it
   */
  @PostMapping("/logout")
